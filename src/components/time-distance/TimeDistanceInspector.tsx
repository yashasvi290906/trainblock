...
export function TimeDistanceInspector({
  ...
  if (!selectedType || !selectedData) {
    return (
      <div className="bg-[#091326] border border-[#162744] rounded-xl p-6 text-center space-y-3 select-none">
  ...
  return (
    <div className="bg-[#091326] border border-[#162744] rounded-xl p-4 space-y-3.5 select-none font-mono text-xs">
      {/* Header */}
      <div className="flex items-center justify-between pb-2 border-b border-[#182a4d]">
        <div className="flex items-center gap-2">
          {selectedType === "TRAIN" && <Train className="w-4 h-4 text-sky-400" />}
          {selectedType === "BLOCK" && <Wrench className="w-4 h-4 text-amber-400" />}
          {selectedType === "CONFLICT" && <AlertTriangle className="w-4 h-4 text-red-400" />}
          {selectedType === "STATION" && <MapPin className="w-4 h-4 text-emerald-400" />}
          {selectedType === "CANDIDATE" && <Layers className="w-4 h-4 text-sky-400" />}
          <h3 className="font-bold text-white uppercase tracking-wider">
            {selectedType} DOSSIER
          </h3>
        </div>
        <span className="text-[10px] bg-[#060c18] px-2 py-0.5 rounded border border-[#152544] text-slate-300">
          INSPECTION
        </span>
      </div>
      ...
      {selectedType === "TRAIN" && (
        <div className="space-y-3">
          <div className="bg-[#060c18] p-3 rounded-lg border border-[#14233e] space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-slate-300 text-[11px] pt-1 border-t border-[#122038]">
              <div>
                <span className="text-slate-500 block">Service:</span>
                <span className="text-white">{(selectedData as TimeDistanceTrain).serviceNumber}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Direction:</span>
                <span className="text-sky-300">{(selectedData as TimeDistanceTrain).direction}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Corridor Path:</span>
                <span className="text-white">SEC → NDL</span>
              </div>
              <div>
                <span className="text-slate-500 block">Status:</span>
                <span className="text-emerald-400 font-semibold">PROTECTED PATH</span>
              </div>
            </div>
          </div>

          <div className="p-2.5 rounded bg-sky-950/30 border border-sky-800/40 text-[11px] text-slate-300 font-sans leading-relaxed">
            <span className="font-bold text-sky-300 font-mono block mb-0.5">
        ...
        <div className="space-y-3">
          <div className="bg-[#060c18] p-3 rounded-lg border border-[#14233e] space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 rounded bg-amber-950/70 border border-amber-800/60 text-amber-300 text-[10px] font-bold">
            ...
            <div className="grid grid-cols-2 gap-2 text-slate-300 text-[11px] pt-1 border-t border-[#122038]">
              ...
              <div>
                ...
                <span className="text-sky-300">ENG + S&T + TRD</span>
              </div>
            </div>
          </div>

          <div className="p-2.5 rounded bg-amber-950/30 border border-amber-800/40 text-[11px] text-slate-300 font-sans leading-relaxed">
            <span className="font-bold text-amber-300 font-mono block mb-0.5">
      ...
      {selectedType === "CONFLICT" && (
        <div className="space-y-3">
          <div className="bg-red-950/30 p-3 rounded-lg border border-red-800/60 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-red-300 text-sm">
                TIMETABLE INTERSECTION
              </span>
              <span className="px-2 py-0.5 rounded bg-red-900 border border-red-700 text-white text-[10px] font-bold">
                CONFLICT
              </span>
            </div>

            <div className="grid grid-cols-2 gap-2 text-slate-300 text-[11px] pt-1 border-t border-red-900/50">
              <div>
              ...
              <div>
                ...
                <span className="text-amber-300 font-bold">REPLAN WINDOW</span>
              </div>
            </div>
          </div>
          ...
          {onReplanToFeasible && (
            <button
              ...
              className="w-full py-2 bg-emerald-600 hover:bg-emerald-500 text-black rounded-lg font-bold text-xs font-mono transition-all flex items-center justify-center gap-2 shadow"
            >
          ...
          <div className="bg-[#060c18] p-3 rounded-lg border border-[#14233e] space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-slate-300 text-[11px] pt-1 border-t border-[#122038]">
              <div>
                ...
                <span className="text-white">SEC–NDL Mainline</span>
              </div>
              <div>
                <span className="text-slate-500 block">Dwell Profile:</span>
                <span className="text-sky-300 font-semibold">2–4 Min Scheduled</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {onApplyCandidate && (
            <button
              ...
              className="w-full py-2 bg-sky-600 hover:bg-sky-500 text-black rounded-lg font-bold text-xs font-mono transition-all flex items-center justify-center gap-2 shadow"
            >
