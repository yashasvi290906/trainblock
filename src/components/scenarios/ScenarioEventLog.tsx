  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-3 font-mono shadow-xs">
      <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-950 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-800">
      ...
      <div className="space-y-1.5 max-h-44 overflow-y-auto pr-1">
        {events.map((evt, idx) => (
          <div
            key={`${evt.id || "evt"}-${idx}`}
            className="p-2 rounded bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80 flex items-start gap-2.5 text-xs text-slate-700 dark:text-slate-300 transition-colors hover:border-slate-300 dark:hover:border-slate-700"
          >
            <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 whitespace-nowrap pt-0.5">
              {evt.timeStr}
            </span>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={cn(
                    ...
                    evt.category === "ALERT"
                      ? "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300 border border-rose-300 dark:border-rose-800/50"
                      : evt.category === "OPERATING"
                      ? "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border border-amber-300 dark:border-amber-800/50"
